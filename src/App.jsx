...
export default function App() {
  ...
  function localScoreAndNotes(ans) {
    ...
    Object.entries(categoryScores).forEach(([cat, scores]) => {
      if (scores.length) {
        ...
        totalScore += avg * w * scores.length;
        totalWeight += w * scores.length;
      }
    });

    let percent = Math.round((totalScore / totalWeight) * 100);
    const perfectAnswers = items.filter((i) => ans[i.id]?.weight === 1).length;
  ...
  function generateDynamicSuggestions(ans, items, categoryScores) {
    ...
    const foundationAvg =
      categoryScores.foundation.length > 0
        ? categoryScores.foundation.reduce((s, v) => s + v, 0) / categoryScores.foundation.length
        : 0;
    const advancedAvg =
      categoryScores.advanced.length > 0
        ? categoryScores.advanced.reduce((s, v) => s + v, 0) / categoryScores.advanced.length
        : 0;

    if (foundationAvg < 0.7) {
      suggestions.push(
        "🎯 STRATEGIC PRIORITY: Focus on cybersecurity fundamentals first - authentication, backups, and training form your security foundation. [Est. 60-90 days, $2000-8000 total investment]"
      );
    }
    if (advancedAvg > foundationAvg + 0.3) {
      suggestions.push(
        "⚖️ BALANCE RECOMMENDATION: Your advanced security exceeds your fundamentals. Strengthen your foundation for better overall protection. [30-day rebalancing focus]"
      );
    }
    const hasCompliance = ans["q9"]?.weight || 0;
    if (hasCompliance < 1) {
      suggestions.push(
        "🏛️ COMPLIANCE ROADMAP: Consider NIST Cybersecurity Framework adoption. Start with 'Identify' and 'Protect' functions. [90-day implementation, consider compliance consultant $5000-15000]"
      );
    }
    const mfaStatus = ans["q1"]?.weight || 0;
    if (mfaStatus < 1) {
      suggestions.push(
        "🔐 IMMEDIATE ACTION (Week 1): Deploy Microsoft Authenticator company-wide. Free solution, 2-hour setup per user. [ROI: Prevents 99.9% of automated attacks]"
      );
    }
    const backupStatus = ans["q3"]?.weight || 0;
    if (backupStatus < 1) {
      suggestions.push(
        "💾 30-DAY GOAL: Implement 3-2-1 backup strategy. Recommended: Veeam + AWS S3 Glacier ($50-200/month). [Business continuity insurance against ransomware]"
      );
    }
  ...
  async function finish(finalAns) {
    ...
    else { securityLevel = "Developing - Needs Attention"; badge = "🔧"; }
  ...
  // Cyber news prefetch (optional hinting)
  ...
  return (
    <div className="page">
      ...
      {/* Midnight Mode CSS */}
      <style jsx>{`
        .midnight-mode { background: #000000 !important; }
        ...
        @keyframes matrixGlow {
          0% { box-shadow: 0 0 30px #00ff4160, inset 0 0 20px #00ff4120; }
          100% { box-shadow: 0 0 50px #00ff4180, inset 0 0 30px #00ff4140; }
        }
      `}</style>
          ...
          <div className="modal__card mobile-menu-card" onClick={(e) => e.stopPropagation()}>
            <div className="modal__head">
              ...
              <button
                ...
                style={{ background: "none", border: "none", color: "var(--text)", fontSize: "1.5rem", cursor: "pointer", padding: "4px 8px" }}
              >
                ×
              </button>
            </div>
            <div className="mobile-menu-items">
              ...
              <button
                ...
                onClick={() => { goNews(); setMobileMenuOpen(false); }}
                onMouseEnter={onPrefetchNews}
                onFocus={onPrefetchNews}
              >
                <span role="img" aria-label="newspaper">📰</span> Latest Cyber News
              </button>
              <button className="btn btn--ghost mobile-menu-btn" onClick={() => { goWhoWeAre(); setMobileMenuOpen(false); }}>
                Who We Are
              </button>
              <button className="btn btn--primary mobile-menu-btn" onClick={() => { start(); setMobileMenuOpen(false); }}>
                Take assessment
              </button>
              <button className="btn btn--ghost mobile-menu-btn" onClick={() => { goResources(); setMobileMenuOpen(false); }}>
                Resources
              </button>
          ...
          <div className="options">
            {q.options.map((opt, i) => (
              <button
                ...
                style={{ opacity: transitioning ? 0.6 : 1, cursor: transitioning ? "not-allowed" : "pointer" }}
              >
                {opt.label}
              </button>
            ))}
      ...
      {view === "results" && result && (
        <section id="report" className="wrap">
          ...
          <div className="category-breakdown" style={{
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(250px, 1fr))", gap: "20px" }}>
              {result.categoryScores && Object.entries({
                ...
                const avg = data.scores.length > 0 ? Math.round((data.scores.reduce((s, v) => s + v, 0) / data.scores.length) * 100) : 0;
                return (
                  <div key={category} style={{ textAlign: "center" }}>
                    <div style={{ fontSize: "24px", marginBottom: "8px" }}>{data.icon}</div>
                    <div className="label-text" style={{ fontSize: "14px", color: theme === "light" ? "#666" : "#a3a7b3", marginBottom: "8px" }}>{category}</div>
                    <div className="category-progress" style={{ width: "100%", height: "8px", background: theme === "light" ? "#e0e0e0" : "#333", borderRadius: "4px", overflow: "hidden", marginBottom: "8px" }}>
                      <div className="category-progress-fill" style={{ width: `${avg}%`, height: "100%", background: `linear-gradient(90deg, ${data.color}AA, ${data.color})`, borderRadius: "4px", transition: "width 0.8s ease" }} />
                    </div>
                    <div className="value-text" style={{ fontSize: "18px", fontWeight: "bold", color: data.color }}>{avg}%</div>
                  </div>
                );
              })}
            </div>
          ...
          <div className="cta screen-only" style={{ marginTop: "32px" }}>
            <button className="btn btn--primary" onClick={() => window.print()} style={{ marginRight: "16px" }}>
              Download Report PDF
            </button>
      ...
      {showEmailModal && (
        <div className="modal" role="dialog" aria-modal="true" aria-label="Email Results">
          <div className="modal__card bubble" style={{ maxWidth: "500px", padding: "2rem" }}>
            <div className="modal__head">
            ...
            <div style={{ marginTop: "1.5rem" }}>
              <p style={{ margin: "0 0 1.5rem", lineHeight: "1.5", color: "var(--text-muted)" }}>
                Thank you for taking our cybersecurity assessment! We can email you a copy of your results.
              </p>
              ...
              <div style={{ marginBottom: "1rem" }}>
                ...
                <input
                  ...
                  style={{
                    ...
                    border: "1px solid var(--border)", background: "var(--panel)", color: "var(--text)", fontSize: "1rem"
                  }}
                  required
                />
              </div>
              ...
              <div style={{ marginBottom: "1.5rem" }}>
                <label style={{ display: "block", marginBottom: "0.5rem", fontWeight: 600 }}>Company Name (Optional)</label>
                <input
                  type="text"
                  value={companyName}
              ...
              <div style={{
                background: "rgba(124, 92, 255, 0.15)", padding: "1rem",
                borderRadius: "8px", marginBottom: "1.5rem",
                border: "1px solid rgba(124, 92, 255, 0.3)"
              }}>
                <p style={{ margin: 0, fontSize: "0.9rem" }}>
                  🛡️ <strong>Privacy Note:</strong> Your email is used only to send your assessment results.
                </p>
              </div>

              <div style={{ display: "flex", gap: "1rem", justifyContent: "flex-end" }}>
                <button className="btn btn--ghost" onClick={() => setShowEmailModal(false)} disabled={emailSending}>
                  Skip
                </button>
                <button
...
function Header({
  ...
  return (
    <header className="topbar">
      <div className="rail">
        <div className="brand" onClick={() => setMobileMenuOpen(!mobileMenuOpen)} style={{ cursor: "pointer" }}>
